...
class HrChecklistCallDoneTable extends Component implements HasForms, HasTable
{
    ...
    public function getCallDoneQuery()
    {
        $this->selectedUser = $this->selectedUser ?? session('selectedUser') ?? auth()->user()->id;
    ...
    public function headerActions(): array
    {
        return [
            Action::make('reset_done_call')
                ...
                ->color('danger')
                ->visible(fn () => $this->getCallDoneQuery()->count() > 0)
                ->modalHeading('Reset Done Calls')
                ->modalDescription('Are you sure you want to reset "Done Calls" to 0 for these HR Checklist leads? This action cannot be undone.')
                ->action(function () {
                    \Illuminate\Support\Facades\DB::beginTransaction();

                    try {
                        $affectedRows = Lead::query()
                            ->whereIn('lead_code', ['HR Checklist', 'HR Checklist (CN)', 'HR Checklist (BM)'])
                            ->where('done_call', '!=', 0)
                            ->where('call_attempt', '!=', 0)
                            ->whereNull('salesperson')
                        ...
                        Notification::make()
                            ...
                            ->body("{$affectedRows} lead(s) updated.")
                            ->success()
                            ->send();
    ...
    {
        return view('livewire.leadowner_dashboard.hr-checklist-call-done-table');
    }
}
