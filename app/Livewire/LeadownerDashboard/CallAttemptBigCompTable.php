...
class CallAttemptBigCompTable extends Component implements HasForms, HasTable
{
    ...
    public function getFollowUpBigCompanyLeads()
    {
        return Lead::query()
            ...
            ->whereNotIn('lead_code', ['Apollo', 'HR Checklist', 'HR Checklist (CN)', 'HR Checklist (BM)'])
            ->where('lead_code', 'NOT LIKE', 'Apollo%')
            ->whereBetween('call_attempt', [1, 10])
            ->where('categories', '!=', 'Inactive') // Exclude Inactive leads
            ->whereNotIn('company_size', ['1-24', '20-24', '1-19']) // Exclude small companies (1-24 and 20-24)
    ...
    public function headerActions(): array
    {
        return [
            Action::make('reset_done_call')
                ...
                ->color('danger')
                ->visible(fn () => $this->getFollowUpBigCompanyLeads()->count() > 0)
    ...
    {
        return view('livewire.leadowner_dashboard.call-attempt-big-comp-table');
    }
}
