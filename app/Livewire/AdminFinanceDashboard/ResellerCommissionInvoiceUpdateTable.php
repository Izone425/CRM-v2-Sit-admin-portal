...
class ResellerCommissionInvoiceUpdateTable extends Component implements HasForms, HasTable
{
    ...
    public function exportExcel()
    {
        ...
        $headers = ['No', 'Date', 'AP Number', 'TT Number', 'Invoice', 'Reseller Name', 'Subscriber Name', 'Amount', 'TT Status', 'Currency', 'Payment Status'];
        ...
        foreach ($records as $index => $record) {
            ...
            // Payment Status
            $paymentStatus = '';
            if (!empty($record->autocount_inv_no)) {
                $aging = DebtorAging::where('invoice_number', $record->autocount_inv_no)->first();
                if (!$aging) {
                    $paymentStatus = 'Not Available';
                } elseif ((float) $aging->outstanding == 0) {
                    $paymentStatus = '';
                } elseif ((float) $aging->outstanding < (float) $aging->invoice_amount) {
                    $paymentStatus = 'Partial Payment';
                } else {
                    $paymentStatus = 'Unpaid';
                }
            }
            $sheet->setCellValue([11, $row], $paymentStatus);
        }
        ...
        $filename = 'reseller-commission-invoice-update-' . now()->format('Y-m-d') . '.xlsx';
    ...
    {
        // Get autocount invoice numbers that have full payment (outstanding = 0) to exclude them
            ...
            ->query(
                CrmInvoiceDetail::query()
                    ->select([
                        ...
                        DB::raw("(SELECT rl.reseller_name FROM crm_reseller_link rl WHERE rl.f_id = CASE WHEN crm_invoice_details.f_company_id = '0000000000' OR crm_invoice_details.f_company_id IS NULL THEN (SELECT tt.f_company_id FROM crm_invoice_details tt WHERE tt.f_invoice_no = TRIM(SUBSTRING(crm_invoice_details.f_desc, LOCATE('TT', crm_invoice_details.f_desc))) LIMIT 1) ELSE crm_invoice_details.f_company_id END LIMIT 1) as reseller_name"),
                        DB::raw("(SELECT rl.f_company_name FROM crm_reseller_link rl WHERE rl.f_id = CASE WHEN crm_invoice_details.f_company_id = '0000000000' OR crm_invoice_details.f_company_id IS NULL THEN (SELECT tt.f_company_id FROM crm_invoice_details tt WHERE tt.f_invoice_no = TRIM(SUBSTRING(crm_invoice_details.f_desc, LOCATE('TT', crm_invoice_details.f_desc))) LIMIT 1) ELSE crm_invoice_details.f_company_id END LIMIT 1) as subscriber_name"),
                    ...
                    ->when(!empty($fullPaymentInvNos), function ($query) use ($fullPaymentInvNos) {
                        ...
                        $query->whereRaw("TRIM(BOTH '\\r\\n' FROM (SELECT tt.f_auto_count_inv FROM crm_invoice_details tt WHERE tt.f_invoice_no = TRIM(SUBSTRING(crm_invoice_details.f_desc, LOCATE('TT', crm_invoice_details.f_desc))) LIMIT 1)) NOT IN ({$placeholders})", $fullPaymentInvNos);
                    })
            )
            ...
            ->columns([
                ...
                TextColumn::make('ac_inv_status')
                    ...
                    ->state(function ($record) {
                        ...
                        if ((float) $aging->outstanding == 0) {
                            return null;
                        }

                        if ((float) $aging->outstanding < (float) $aging->invoice_amount) {
                ...
                TextColumn::make('reseller_name')
                    ...
                    ->searchable(query: function (Builder $query, string $search): Builder {
                        return $query->orWhereRaw(
                            "(SELECT rl.reseller_name FROM crm_reseller_link rl WHERE rl.f_id = CASE WHEN crm_invoice_details.f_company_id = '0000000000' OR crm_invoice_details.f_company_id IS NULL THEN (SELECT tt.f_company_id FROM crm_invoice_details tt WHERE tt.f_invoice_no = TRIM(SUBSTRING(crm_invoice_details.f_desc, LOCATE('TT', crm_invoice_details.f_desc))) LIMIT 1) ELSE crm_invoice_details.f_company_id END LIMIT 1) LIKE ?",
                            ['%' . $search . '%']
                        );
                    })
                ...
                TextColumn::make('subscriber_name')
                    ...
                    ->searchable(query: function (Builder $query, string $search): Builder {
                        return $query->orWhereRaw(
                            "(SELECT rl.f_company_name FROM crm_reseller_link rl WHERE rl.f_id = CASE WHEN crm_invoice_details.f_company_id = '0000000000' OR crm_invoice_details.f_company_id IS NULL THEN (SELECT tt.f_company_id FROM crm_invoice_details tt WHERE tt.f_invoice_no = TRIM(SUBSTRING(crm_invoice_details.f_desc, LOCATE('TT', crm_invoice_details.f_desc))) LIMIT 1) ELSE crm_invoice_details.f_company_id END LIMIT 1) LIKE ?",
                            ['%' . $search . '%']
                        );
                    })
                    ->formatStateUsing(fn ($state) => strtoupper($state ?? ''))
    ...
    {
        return view('livewire.admin-finance-dashboard.reseller-commission-invoice-update-table');
    }
}
