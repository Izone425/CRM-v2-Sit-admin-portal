...
namespace App\Livewire\SupportDashboard;
...
class SupportCommentPendingFe extends Component implements HasForms, HasTable
{
    ...
    public function getCompletedTicketsQuery()
    {
        ...
        // Group selection — only tickets where requestor is a Support / Technician user
        if ($this->selectedUser === 'all-support') {
            $supportEmails = \App\Models\User::whereIn('role_id', [8, 9])->pluck('email')->toArray();
            $query->whereHas('requestor', fn ($q) => $q->whereIn('email', $supportEmails));
            return $query;
        }

        // Specific user selected — only scope when that user is Support or Technician
        if (is_numeric($this->selectedUser)) {
            $user = \App\Models\User::find($this->selectedUser);
            if ($user && in_array($user->role_id, [8, 9], true)) {
                $query->whereHas('requestor', fn ($q) => $q->where('email', $user->email));
            }
            return $query;
        }

        // Fallback: scope to the logged-in user only if they are Support or Technician
        $currentUser = auth()->user();
        if ($currentUser && in_array($currentUser->role_id, [8, 9], true)) {
    ...
    {
        return view('livewire.support_dashboard.support-comment-pending-fe');
    }
}
