...
class ProcessDataMyrStatsCards extends Component
{
    public $newStats;
    ...
    protected function getStatsForProgress(array $progressValues): array
    {
        try {
            ...
            Log::error('Error fetching renewal stats for '.implode(',', $progressValues).': '.$e->getMessage());
        ...
        try {
            ...
            $renewals = $renewalsQuery->get()
                ->filter(function ($renewal) use ($startOfMonth, $endOfMonth, $today) {
                    return RenewalDataMyr::where('f_company_id', $renewal->f_company_id)
    ...
    {
        return view('livewire.admin-renewal-dashboard.process-data-myr-stats-cards');
    }
}
