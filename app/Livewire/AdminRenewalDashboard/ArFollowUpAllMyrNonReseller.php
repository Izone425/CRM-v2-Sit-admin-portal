...
class ArFollowUpAllMyrNonReseller extends Component implements HasForms, HasTable
{
        ...
        return $table
            ...
            ->headerActions([
                Action::make('exportExcel')
                    ...
                    ->color('success')
                    ->action(function () {
                        $records = $this->getOverdueRenewals()->get();

                        $spreadsheet = new Spreadsheet();
                        $sheet = $spreadsheet->getActiveSheet();
                        $sheet->setTitle('All Follow Ups');

                        $headers = ['No', 'Company Name', 'Status', 'Expiry Date', 'Reseller', 'Pending Days', 'Follow Up Date'];
                        ...
                        foreach ($records as $index => $record) {
                            $expiryDate = self::getEarliestExpiryDate($record->f_company_id);
                            $reseller = $this->getCachedReseller($record->f_company_id);
                            $resellerStatus = $reseller && $reseller->f_rate ? 'Yes' : 'No';
                            $pendingDays = $this->getWeekdayCount($record->follow_up_date, now());

                            $sheet->setCellValue("A{$row}", $index + 1);
                            $sheet->setCellValue("B{$row}", $record->company_name ?? '');
                            $sheet->setCellValue("C{$row}", $statusMap[$record->renewal_progress] ?? ucfirst(str_replace('_', ' ', $record->renewal_progress)));
                            $sheet->setCellValue("D{$row}", $expiryDate ? Carbon::parse($expiryDate)->format('d M Y') : 'N/A');
                            $sheet->setCellValue("E{$row}", $resellerStatus);
                            $sheet->setCellValue("F{$row}", $pendingDays . ' Days');
                            $sheet->setCellValue("G{$row}", $record->follow_up_date ? Carbon::parse($record->follow_up_date)->format('d M Y') : '');
                        ...
                        $fileName = 'follow_up_all_myr_non_reseller_' . now()->format('Ymd_His') . '.xlsx';
            ...
            ->columns([
                ...
                TextColumn::make('earliest_expiry_date')
                    ...
                    ->sortable(query: function ($query, string $direction) {
                        $orderedSourceIds = [];
                        try {
                            $orderedSourceIds = DB::connection('frontenddb')
                                ...
                                ->select('f_company_id', DB::raw('MIN(f_expiry_date) as earliest'))
                                ->groupBy('f_company_id')
                                ->orderBy('earliest', $direction)
                                ->pluck('f_company_id')
                                ->toArray();
                        } catch (\Exception $e) {
                            Log::error('Error sorting by earliest expiry: ' . $e->getMessage());
                            return $query;
                        }

                        if (empty($orderedSourceIds)) {
                            return $query;
                        }

                        $orderedIds = [];
                        foreach ($orderedSourceIds as $cid) {
                            $without = (string) (int) $cid;
                            $with = str_pad($without, 10, '0', STR_PAD_LEFT);
                            $orderedIds[] = $without;
                            if ($with !== $without) {
                                $orderedIds[] = $with;
                            }
                        ...
                        return $query->orderByRaw(
                            "(FIELD(f_company_id, {$placeholders}) = 0) ASC, FIELD(f_company_id, {$placeholders}) ASC",
                            array_merge($orderedIds, $orderedIds)
                        );
                    })
                    ->formatStateUsing(function ($state, $record) {
            ...
            ->actions([
                ...
                ->button()
                ->color('info') // Blue color for non-reseller
                ->label('Actions')
            ]);
    }
    ...
    {
        return view('livewire.admin_renewal_dashboard.ar-follow-up-all-myr-non-reseller');
    }
