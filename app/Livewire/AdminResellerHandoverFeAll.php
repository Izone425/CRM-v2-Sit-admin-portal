...
class AdminResellerHandoverFeAll extends Component implements HasForms, HasTable
{
    ...
    {
        return $table
            ->query(ResellerHandoverFe::query()->orderBy('created_at', 'desc'))
            ->columns([
                TextColumn::make('fe_id')
                    ...
                    ->searchable(query: function (Builder $query, string $search): Builder {
                        return $query->whereRaw("CONCAT('FE', DATE_FORMAT(created_at, '%y%m'), '-', LPAD((SELECT COUNT(*) FROM reseller_handover_fes AS rh2 WHERE YEAR(rh2.created_at) = YEAR(reseller_handover_fes.created_at) AND MONTH(rh2.created_at) = MONTH(reseller_handover_fes.created_at) AND rh2.id <= reseller_handover_fes.id), 4, '0')) LIKE ?", ["%{$search}%"]);
                    }),
    ...
    {
        return view('livewire.admin-reseller-handover-fe-all');
    }
}
