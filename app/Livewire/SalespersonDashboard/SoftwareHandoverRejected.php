...
class SoftwareHandoverRejected extends Component implements HasForms, HasTable
{
                            ...
                            Forms\Components\Actions::make([
                                Forms\Components\Actions\Action::make('export_invoice_info')
                                    ...
                                    ->url(function (?SoftwareHandover $record = null) {
                                        ...
                                        if (!$leadId) return '#';
                                        return route('software-handover.export-customer', ['lead' => \App\Classes\Encryptor::encrypt($leadId)]);
                                    })
                                    ->openUrlInNewTab(),
                            ])
            ...
            Section::make('Step 3: Implementation Details')
                ->schema([
                    Forms\Components\Repeater::make('implementation_pics')
                        ...
                        ->minItems(1)
                        ->itemLabel(fn() => __('Person In Charge'))
                        ->columns(2)
                        ...
                        ->schema([
                            Select::make('proforma_invoice_product')
                                ...
                                ->options(function (?SoftwareHandover $record = null) {
                                    ...
                                    $availableQuotations = Quotation::where('lead_id', $leadId)
                                        ...
                                        ->whereHas('items', function ($query) use ($moduleProductIds) {
                                ...
                                ->afterStateUpdated(function (Forms\Set $set, ?array $state, CategoryService $category) {
                                    ...
                                    $highestQuantity = QuotationDetail::whereIn('quotation_id', $state)
                                        ->max('quantity');
                                    if ($highestQuantity) {
                        ...
                        ->schema([
                            Textarea::make('renewal_note')
                                ...
                                ->default(function (?SoftwareHandover $record = null) {
                                    ...
                                    $latestNote = RenewalNote::where('lead_id', $record->lead_id)
                                        ->latest()
                                        ->first();
                    ...
                    Grid::make(1)
                        ->schema([
                            Select::make('reseller_id')
                                ...
                                ->options(function () {
                                    return Reseller::orderBy('company_name')
    ...
    private function updateHrdfGrantIdRepeater(callable $set, callable $get, ?array $state): void
    {
        ...
        $quotations = Quotation::whereIn('id', $selectedHrdfInvoices)
            ->with(['subsidiary', 'lead.companyDetail'])
            ->get();
                        ...
                        ->action(function (SoftwareHandover $record, array $data): void {
                            ...
                            if (!empty($renewalNote)) {
                                try {
                                    ...
                                    \Illuminate\Support\Facades\Log::error('Failed to update renewal note', [
                                        'error' => $e->getMessage(),
                                        'lead_id' => $record->lead_id,
                                    ]);
                                }
                            ...
                            Notification::make()
                                ->title('Software handover updated and resubmitted successfully')
                                ->success()
                                ->send();
                        }),
                ])->button()
    ...
    {
        return view('livewire.salesperson_dashboard.software-handover-rejected');
    }
}
