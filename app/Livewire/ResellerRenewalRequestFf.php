...
class ResellerRenewalRequestFf extends Component
{
    // Kept for backward compatibility reference
    private const ADVANCED_MODULE_RESELLER_ID = '0000000934';
    ...
    public function getCanManageAdvancedModulesProperty(): bool
    {
        ...
        return ($reseller->advanced_modules ?? 'disable') === 'enable';
    }
    ...
    public function viewLicense()
    {
        ...
        $canManageAdvanced = $this->canManageAdvancedModules;
    ...
    public function submitRequest()
    {
        ...
        if ($this->canManageAdvancedModules) {
            $this->validate([
                'vms' => 'required|integer|min:0',
                'patrol' => 'required|integer|min:0',
                'access' => 'required|integer|min:0',
                'fcc' => 'required|integer|min:0',
            ]);
        } else {
            $this->vms = 0;
            $this->patrol = 0;
        ...
        if (
            ...
            $this->headcountError = 'Please enter at least 1 headcount for any product (Attendance, Leave, Claim, Payroll, QF Master, VMS, Patrol, Access, or FCC).';
            return;
        }

        $reseller = Auth::guard('reseller')->user();

        ResellerHandoverFf::create([
            ...
            'vms_qty' => $this->vms,
            'patrol_qty' => $this->patrol,
            'access_qty' => $this->access,
            'fcc_qty' => $this->fcc,
        ...
        $this->dispatch('notify', message: 'FF request submitted successfully!', type: 'success');
        $this->closeModal();
    }

    public function render()
    {
        return view('livewire.reseller-renewal-request-ff', [
