<style>
    ...
    .section-title {
        margin-bottom: 0.75rem;
        font-weight: 500;
        color: #0694a2; /* Cyan for attendance module */
        font-size: 0.875rem;
    }
...
@php
    ...
    $clockingScheduleFiles = collect($files)->where('subcategory', 'clocking_schedule');
@endphp

<div class="section-container">
    <h4 class="section-title">CLOCKING SCHEDULE</h4>
    @if($clockingScheduleFiles->count() > 0)
        <div class="files-grid">
            @foreach($clockingScheduleFiles as $file)
                <div class="file-item">
                    <div class="file-name">
                        @php
                            $originalName = basename($file->filename);
                            $extension = pathinfo($originalName, PATHINFO_EXTENSION);
                            $shortName = "Clocking Schedule " . $loop->iteration;
                        @endphp
