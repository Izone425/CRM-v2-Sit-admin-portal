    ...
    <div class="email-container">
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <br><span class="info-label" style="color: #dc2626;">Reason Reject:</span> <span style="color: #dc2626;">{{ $rejectReason ?? 'N/A' }}</span>
                @endif
            </div>
            ...
            <div class="info-row">
                ...
                <br><span class="info-label">Admin Remark:</span> {{ !empty($handover->admin_reseller_remark) ? $handover->admin_reseller_remark : 'N/A' }}
                @endif
            ...
            @endif
            @if(($status === 'completed' && isset($financePaymentSlipUrl) && $financePaymentSlipUrl) || ($status === 'completed' && isset($selfBilledEInvoiceUrl) && $selfBilledEInvoiceUrl))
            <div class="info-row">
                @if(isset($financePaymentSlipUrl) && $financePaymentSlipUrl)
                <span class="info-label">Finance Payment Slip:</span>
                <a href="{{ $financePaymentSlipUrl }}" style="color: #431fa1; text-decoration: underline; font-weight: bold;">View Document</a>
                @endif
                @if(isset($selfBilledEInvoiceUrl) && $selfBilledEInvoiceUrl)
                @if(isset($financePaymentSlipUrl) && $financePaymentSlipUrl)<br>@endif
                <span class="info-label">Self Billed E-Invoice:</span>
                <a href="{{ $selfBilledEInvoiceUrl }}" style="color: #431fa1; text-decoration: underline; font-weight: bold;">View Document</a>
                @endif
            </div>
            @endif
        </div>
        ...
        @elseif($category !== 'Bill as Reseller')
            @if($status === 'pending_quotation_confirmation' && isset($proceedUrl) && $proceedUrl)
